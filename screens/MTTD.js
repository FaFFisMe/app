...
const MTTD = () => {
  ...
  const processData = (data) => {
    ...
    if (data != null) {
      data.forEach(value => {
        const intValue = value ? parseInt(convertMillisToTime(value).secondsConv) : 0;
        for (let i = 0; i < ranges.length; i++) {
          if (intValue < ranges[i]) {
            datetime[i] = datetime[i] + intValue.toString() + " s.\n";
            counts[i]++;
            break;
          }
        }
      ...
      return counts.map((value, index) => ({ value, 
        label: index < ranges.length-1 ?  ranges[index] + " s." : "",
        frontColor: index == "0" || index == "4" ?  "#B22222" : "#74B72E",
        topLabelComponent: () => (
          <Text style={{color: 'lightgreen', fontSize: 18, marginBottom: 6}}>{value}</Text>
        ),
  ...
  const lineData1 = processData(jsonData.TTDall);
      ...
      <View style={[styles.frameGroup, styles.frameGroupFlexBox]}>
          ...
          <View style={styles.frameView}>
            <View style={styles.mttdavghrWrapper}>
              <Text style={[styles.mttdavghr, styles.mttdavgmTypo]}>
                {convertMillisToTime(jsonData && jsonData.TTDavg).hours}
              </Text>
            </View>
        ...
        <View style={[styles.frameParent1, styles.frameParentLayout]}>
          ...
          <View style={styles.frameView}>
            <View style={styles.mttdavghrWrapper}>
              <Text style={[styles.mttdavghr, styles.mttdavgmTypo]}>
                {convertMillisToTime(jsonData && jsonData.TTDmax).hours}
              </Text>
            </View>
            <View style={[styles.wrapper, styles.wrapperPosition]}>
              <Text style={styles.textTypo}>:</Text>
            ...
            <View style={styles.mttdavgmWrapper}>
              <Text style={[styles.mttdavgm, styles.mttdavgmTypo]}>
                {convertMillisToTime(jsonData && jsonData.TTDmax).minutes}
              </Text>
            </View>
            ...
            <View style={[styles.mttdavgsWrapper, styles.wrapperPosition]}>
              <Text style={[styles.mttdavgs, styles.mttdavgmTypo]}>
                {convertMillisToTime(jsonData && jsonData.TTDmax).seconds}
              </Text>
            </View>
          </View>
        </View>
        <View style={[styles.frameParent3, styles.frameParentLayout]}>
          ...
          <View style={styles.frameView}>
            <View style={styles.mttdavghrWrapper}>
              <Text style={[styles.mttdavghr, styles.mttdavgmTypo]}>
                {convertMillisToTime(jsonData && jsonData.TTDmin).hours}
              </Text>
            </View>
            <View style={[styles.wrapper, styles.wrapperPosition]}>
              <Text style={styles.textTypo}>:</Text>
            ...
            <View style={styles.mttdavgmWrapper}>
              <Text style={[styles.mttdavgm, styles.mttdavgmTypo]}>
                {convertMillisToTime(jsonData && jsonData.TTDmin).minutes}
              </Text>
            </View>
            <View style={[styles.frame, styles.framePosition]}>
              <Text style={styles.textTypo}>:</Text>
            ...
            <View style={[styles.mttdavgsWrapper, styles.wrapperPosition]}>
              <Text style={[styles.mttdavgs, styles.mttdavgmTypo]}>
                {convertMillisToTime(jsonData && jsonData.TTDmin).seconds}
              </Text>
            </View>
